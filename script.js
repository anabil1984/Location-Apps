...
let imageryLayer = L.tileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}",{
...
const baseLayers = {
  "imagery satellite": imageryLayer,
  "OpenStreetMap":OSM_Mapnik,
  "DarkMatter" :CartoDB_DarkMatter
};

const map = L.map('map').setView([30.128611,31.242222], 13);
...
map.on('draw:created', function (e) {
  editableLayers.addLayer(L.marker([e.layer. _latlng.lat,e.layer. _latlng.lng]))
  arrPure.push(L.latLng(e.layer._latlng.lat,e.layer. _latlng.lng));  
...
function handleRouting(){
  let i=editableLayers._layers
  console.log(i);
 arrPure=[];
for (const x in i){
  arrPure.push(L.latLng(i[x]._latlng.lat,i[x]._latlng.lng));  
}

if(arrPure.length<=1)
{
  alert("You must enter at least 2 points");
return 0;
}
else
  {
    ...
    L.Routing.control({
      ...
      autoRoute:true, // if false, route will no be drawn!
      }).addTo(map);
...
// ------ getLocation using pure JS ----------------
// ---- this is commented because a plugin is already used----
